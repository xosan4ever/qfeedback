...
window.Feedback = function( options ) {
    ...
    // default properties
    options.label = options.label || "Отправить сообщение";
    options.header = options.header || "Заметили ошибку?";
    options.url = options.url || "/quickfeedback/index.php";
    options.adapter = options.adapter || new window.Feedback.XHR( options.url );
    
    options.nextLabel = options.nextLabel || "Продолжить";
    options.reviewLabel = options.reviewLabel || "Предпросмотр";
    options.sendLabel = options.sendLabel || "Отправить";
    options.closeLabel = options.closeLabel || "Закрыть";
    
    options.messageSuccess = options.messageSuccess || "Ваше сообщение успешно отправлено. Спасибо!";
    options.messageError = options.messageError || "Ошибка при отправке.";
...
window.Feedback.Form = function( elements ) {
    ...
    this.elements = elements || [{
        type: "textarea",
        name: "Issue",
        label: "Пожалуйста кратко опишите проблему",
        required: false
    }];
...
window.Feedback.Screenshot.prototype.start = function( modal, modalHeader, modalFooter, nextButton ) {
    ...
    if ( this.h2cDone ) {
        ...
        blackoutButton = element("a", "Закрасить"),
        highlightButton = element("a", "Выделить"),
        previousElement;
        ...
        this.dom.appendChild( element("p", "Тут можно подсветить или закрасить блоки сайта.") );
